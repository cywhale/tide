...
dask-worker tcp://localhost:8786 --memory-limit 4GB &

# run API server
## localhost: gunicorn tide_app:app -k uvicorn.workers.UvicornWorker -b 127.0.0.1:8040 --timeout 120
gunicorn tide_app:app -w 4 -k uvicorn.workers.UvicornWorker -b 127.0.0.1:8040 --keyfile conf/privkey.pem --certfile conf/fullchain.pem --reload --timeout 120
