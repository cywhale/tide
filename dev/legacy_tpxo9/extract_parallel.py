...
from src.model_extract import extract_ATLAS_pytmd  # extract_ATLAS_v2
...
ShallowLimit = 0
extract_method = "extract_pytmd"  # 'extract_Atlas_v2'
enExtrapolate = True
# concurrent processors used
maxWorkers = 4 if extract_method == "extract_Atlas_v2" else 3
...
def process_chunk(
    ...
    save_to_zarr(
        amp, ph, c, amp_var, ph_var, lon_chunk, lat_chunk, chunk_file, group_name, mode
    )
    return chunk_num
...
def tpxo2zarr(
    ...
    if global_grid:
        ...
        if (
            len(lon) not in lon_range
        ):  # Ensure last element is included if it's not already
...
def combine_chunk_zarr(lon_range, lat_range, chunk_file):
    ...
    for lat_idx in range(num_lat_chunks):
        for lon_idx in range(num_lon_chunks):
            ...
            assert os.path.exists(
                os.path.join(chunk_file, chunk_label)
            ), f"Chunk {chunk_label} is missing!"

    # Organize the chunks into a 2D matrix
    chunks_matrix = [
        [
            xr.open_zarr(
                chunk_file, group=f"chunk_{lat_idx * num_lon_chunks + lon_idx}"
            )
            for lon_idx in range(num_lon_chunks)
        ]
        for lat_idx in range(num_lat_chunks)
    ...
    # Concatenate chunks
    concat_along_lon = [
        xr.concat(lon_chunks, dim="lon") for lon_chunks in chunks_matrix
    ]
...
def main():
    ...
    for type in TYPE:
        ...
        constituents = ATLAS.read_constants(
            ...
            model_files,
            type=type,
            compressed=tpxo_model.compressed,
        )
        print("model type is: ", type, " constituents: ", constituents)

        if global_grid:
            lonc, latc, bathy_c = read_netcdf_grid(BATHY_gridfile, variable=type)
    ...
    if doneFlag:
        print("After type extraction, do combine_chunk of final stage...")
