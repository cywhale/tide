...
## Global variables
maxWorkers = 12
MAX_CLUSTERS = 24 #None if need test
...
def save_chunk_to_zarr(store, ds, i, j, chunk_size):
    print(f"Started saving chunk {i}, {j}")
    print(f"Process {os.getpid()} attempting to acquire lock")
    with lock:
        ...
        try:
            ...
            print(f"About to save chunk ({i}:{i+chunk_size}, {j}:{j+chunk_size})")

            # Define the region as a dictionary
            # region_dict = {
            #   'lat': slice(i, i+chunk_size),
            #   'lon': slice(j, j+chunk_size),
            #   'constituents': slice(None)
            #}
            # Check if store has the expected Zarr structure
            #if '.zgroup' not in store or '.zarray' not in store:
            #    print(f"Zarr store at chunk {i}, {j} appears to be missing expected metadata.")
            # Save the entire chunk at once
            #ds_chunk.to_zarr(store, mode='a', compute=True, region=region_dict)

            # Directly save each variable in ds_chunk to the appropriate location in Zarr store
            for var_name, variable in ds_chunk.data_vars.items():
                print(f"About to save variable {var_name} for chunk ({i}:{i+chunk_size}, {j}:{j+chunk_size})")
                store[var_name][i:i+chunk_size, j:j+chunk_size, :] = variable.values
                print(f"Finished saving variable {var_name} for chunk ({i}:{i+chunk_size}, {j}:{j+chunk_size})")

            print(f"Finished saving for chunk ({i}:{i+chunk_size}, {j}:{j+chunk_size})")
            print(f"Process {os.getpid()} releasing lock")

        except Exception as e:
            print(f"Error processing saving chunk {i}, {j}: {e}")
            #print(f"Error processing saving chunk {i}, {j}: {e}\n", traceback.format_exc())
            if hasattr(e, 'args') and e.args:
                print(f"Additional error info: {' '.join(map(str, e.args))}")


def save_dataset_parallel(ds, store, chunk_size, max_workers=4):
    tasks = []
    with ProcessPoolExecutor(max_workers=max_workers, initializer=initialize_lock, initargs=(lock,)) as executor:
        try:
            ...
            for future in as_completed(tasks):
                try:
                    future.result()  # This will raise any exception that occurred during the processing.
                except Exception as e:
                    print(f"Error occurred during parallel saving of a chunk: {e}")
        except Exception as e:
            print(f"Error occurred while submitting parallel saving tasks: {e}")
...
def main():
    ...
    if False: #SAVE_SMALL_DATA:
        ...
        save_dataset_parallel(ds, store, chunk_size=chunk_size, max_workers=4) #use less workers
